...
// Captured from a real session: running a slash command before typing leaves
// the whole command transcript in the user's message. On that turn the noise is
// most of the text, and several blocks talk about models and configuration —
// the exact subject most likely to mislead a complexity judgement.
func TestNewTurnPromptStripsTheLocalCommandTranscript(t *testing.T) {
	body := decode(t, `{"tools":[{"name":"Read"}],"messages":[{"role":"user","content":
		"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to.</local-command-caveat>\n<command-name>/model</command-name>\n<command-message>model</command-message>\n<command-args></command-args>\n<local-command-stdout>Set model to `+"`jev-router`"+` and saved as your default for new sessions</local-command-stdout>\nme diz sobre o que é este repo"}]}`)

	got := NewTurnPrompt(body)
	if got != "me diz sobre o que é este repo" {
		t.Errorf("got %q, want only the user's own words", got)
	}
	for _, leak := range []string{"jev-router", "command-name", "Caveat", "local-command"} {
		if strings.Contains(got, leak) {
			t.Errorf("the router would judge CLI noise: %q leaked into the prompt", leak)
		}
	}
}
